...
*   **`api/chat.py`**: Handles incoming chat messages, routing them to the AI responder and enforcing post-generation validation before returning the answer to the user.
...
*   **`ai/responder.py`**: Uses **OpenAI GPT-4o-mini** to power the conversational chatbot. It is fed the user's aggregated financial profile as context, allowing it to answer specific questions about their taxes.
*   **`ai/validator.py`**: The crucial safety net. This script scans output generated by `responder.py` using Regex. If the LLM mentions a Rupee amount (₹) that does not perfectly match a figure hardcoded in the user's database profile, the response represents a hallucination and is violently blocked from reaching the user.
...
*   **`schemas/documents.py` & `schemas/chat.py`**: Defines Pydantic v2 models. These dictate exactly what standard an incoming API request must adhere to, and what structure the outgoing response will take.
...
6.  **Chat**: User asks "How much tax do I owe?". `ai/responder.py` checks the `FinancialProfile` and writes an answer. `ai/validator.py` ensures the number in the answer exactly matches the engine's calculation before sending it.
